...
def metric_model_builder(data_format, model_desc):
  ...
  def model(inputs, is_training):
    ...
    for layer in model_desc:
      ...
      name, kernel, stride, fan_in, fan_out = vals
      if op == "conv":
...
def residual_block(inputs, filters, is_training, resize_shape,
  ...
  if use_projection:
    ...
    if SK_RATIO > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
      ...
      if resize_shape is not None:
        shortcut = tf.image.resize(inputs, resize_shape)
      else:
        shortcut = inputs
      shortcut = conv2d_fixed_padding(
          inputs=shortcut, filters=filters, kernel_size=1, strides=1,
          data_format=data_format)
...
def bottleneck_block(inputs, filters, is_training, resize_shape,
                     use_projection=False, data_format='channels_last',
                     dropblock_keep_prob=None, dropblock_size=None):
...
def block_group(inputs, filters, block_fn, blocks, resize_shape, is_training, name,
  ...
  inputs = block_fn(inputs, filters, is_training, resize_shape=resize_shape,
...
def resnet_v1_generator_decoder(block_fn, layers, width_multiplier,
  ...
  def model(inputs, endpoints, is_training, skip=True):
    ...
    inputs = block_group(
        ...
        blocks=layers[3], resize_shape=resize_shape, is_training=is_training,
    ...
    inputs = block_group(
        ...
        blocks=layers[2], resize_shape=resize_shape, is_training=is_training,
    ...
    inputs = block_group(
        ...
        blocks=layers[1], resize_shape=resize_shape, is_training=is_training,
    ...
    inputs = block_group(
        ...
        blocks=layers[0], resize_shape=resize_shape, is_training=is_training, #, strides=2
    ...
    if cifar_stem:
      ...
      resize_shape = [resize_shape[0]*2,resize_shape[1]*2]
      inputs = tf.image.resize(inputs, resize_shape)
      inputs = tf.identity(inputs, 'resize_L-1')
...
def learned_metric_v1(metric_channels, data_format='channels_last'):
  ...
  #   {'conv': ['conv2_1', 3, 1, 128, 256]},  # noqa name, kernel, stride, in, out
  ...
  # ]
  DEFAULT_METRIC_MODEL = [
    {'conv': ['conv1_1', 3, 1, 3, 24]},  # noqa name, kernel, stride, in, out
    {'conv': ['conv1_2', 3, 1, 64, 32]},  # noqa name, kernel, stride, in, out
    {'pool': ['pool1', 2, 2, 128, 32]},  # noqa name, kernel, stride, in, out
    {'conv': ['conv2_1', 3, 1, 128, 32]},  # noqa name, kernel, stride, in, out
    {'conv': ['conv2_2', 3, 1, 128, 32]},  # noqa name, kernel, stride, in, out
    {'conv': ['conv2_3', 3, 1, 128, 32]},  # noqa name, kernel, stride, in, out
    {'conv': ['conv2_4', 3, 1, 128, 32]},  # noqa name, kernel, stride, in, out
    {'conv': ['conv2_5', 3, 1, 128, metric_channels]},  # noqa name, kernel, stride, in, out
    # {'global_mean': ['gap', 3, 1, 128, 128]},  # noqa name, kernel, stride, in, out
  ]
...
# out = resnet_decoder_v1(18, 1)(image, is_training=True)
